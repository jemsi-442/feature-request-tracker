...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(data.message || t("auth.passwordUpdated"));
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(165deg,#0f172a_0%,#123b38_48%,#2b3444_100%)] px-4 py-12">
      <div className="max-w-5xl mx-auto grid lg:grid-cols-2 rounded-3xl overflow-hidden border border-white/15 shadow-2xl shadow-black/30">
        <aside className="hidden lg:flex flex-col justify-between bg-[radial-gradient(circle_at_bottom_left,rgba(245,158,11,0.24),transparent_42%),#111827] p-10 text-white">
          <div>
            <p className="text-xs uppercase tracking-[0.22em] text-amber-100">{t("auth.resetBadge")}</p>
            <h1 className="mt-4 text-4xl font-black leading-tight">{t("auth.resetBrandTitle")}</h1>
            <p className="mt-4 text-slate-300">{t("auth.resetBrandDesc")}</p>
          </div>
          <p className="text-sm text-slate-400">{t("auth.resetBrandFoot")}</p>
        </aside>
        ...
        >
          ...
          <p className="mt-1 text-slate-500">{t("auth.resetIntro")}</p>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div>
              <label className="block mb-1 text-sm font-medium text-slate-700">{t("auth.newPassword")}</label>
